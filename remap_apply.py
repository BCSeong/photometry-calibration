...
def _load_map_pair(map_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    mx_path = mx_matches[0]  # 첫 번째 매칭된 파일 사용
    my_path = my_matches[0]  # 첫 번째 매칭된 파일 사용
...
# deprecated: use _write_bmp instead
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    for sub in folders:
        ...
        for img_path in imgs:
            ...
            out = out_root / sub.name / f"{args.label}_{img_path.stem}.bmp"
            #out = out_root / f"{args.label}_{img_path.stem}.bmp"
            _write_bmp(out, remap)
